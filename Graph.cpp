...
(const unordered_map<string, vector<PCPart>>& traversalResult) {
    ...
    for (const auto& partType : traversalResult) {
        if (partType.first == "psu") {
            PCPart randomPSU = selectRandomPart(partType.second);
            if (arePartsCompatible(randomBuild["gpu"], randomPSU))
                randomBuild[partType.first] = randomPSU;
        }
        else
